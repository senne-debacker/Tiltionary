...
export default function WaitingScreen({ roomCode, role }) {
  ...
  const startGame = () => {
    ...
    // We updaten nu de status én we zetten het woord erin!
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>
        Kamer Code: <Text style={styles.highlight}>{roomCode}</Text>
      </Text>

      <View style={styles.listContainer}>
        <Text style={styles.subtitle}>
          Spelers in de lobby ({players.length}):
        </Text>
        <FlatList
          data={players}
      ...
      {role === "guest" && (
        <View style={styles.rulesContainer}>
          ...
          <Text style={styles.rulesText}>
            • Raad in de chat, hoe sneller hoe meer punten (tot 1000pt).
          </Text>
          <Text style={styles.rulesText}>
            • Top 3 snelste raders krijgen een bonus!
          </Text>
          <Text style={styles.rulesText}>
            • De tekenaar krijgt extra punten als het woord geraden wordt.
          </Text>
        </View>
      )}
      ...
      {role === "host" ? (
        <TouchableOpacity
          style={[
            styles.startButton,
            players.length < 2 ? styles.disabledButton : null,
          ]}
          onPress={startGame}
