...
export default function GameScreen({ role, roomCode, wordToDraw, appState, paths, setPaths, setAppState }) {
  ...
  // Functie om de Sandbox af te sluiten
  const quitSolo = () => {
    setPaths([]);
    setAppState('lobby');
  };
...
useEffect(() => {
    ...
    const subscription = Accelerometer.addListener(({ x, y, z }) => {
      ...
      if (gForce > 2.2 && drawStateRef.current === 'drawing') {
        ...
        setPaths([]);
        // FIX 1: Voeg '/drawing' toe aan het pad als je het canvas wist
        if (role === 'host' && roomCode) set(ref(db, `rooms/${roomCode}/drawing/paths`), []); 
        return;
      }
        ...
        if (lastPoint && Math.hypot(newX - lastPoint.x, newY - lastPoint.y) > 2) {
          ...
          if (role === 'host' && roomCode) {
            ...
            if (now - lastSyncTime.current > 100) {
              // FIX 2: Voeg '/drawing' toe aan het pad voor de Delta Updates
              set(ref(db, `rooms/${roomCode}/drawing/paths/${lastPathIndex}`), {
  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.container}>
      ...
      <View 
        style={[styles.canvas, { backgroundColor: (role === 'host' || role === 'solo') && drawState === 'waiting' ? '#2c2c2e' : '#1c1c1e' }]}
...
const styles = StyleSheet.create({
  ...
  soloHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', width: '100%', paddingHorizontal: 30 },
  quitButton: { backgroundColor: '#FF3B30', paddingHorizontal: 15, paddingVertical: 8, borderRadius: 10 },
